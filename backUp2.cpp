...
int chanceProbability(int row, int col, int n)
{
    float baseChance = 0.02f, adaptive, finalChance, gradient = 4.8f;
    adaptive = pow((float)(row + col) / (2.f * (n - 1)), gradient);
        ...
        if (cut_horizontalORvertiacal == 1)
        {
            for (int row = 1; row < n - 1; ++row)
            {
                for (int col = 1; col < n - 2; ++col)
                {
                    ...
                    {
                        if ((rand() % 100) < chanceProbability(row, col, n))
                        {
                            maze[row][col + 1] = 1;
                            ++count;
                            ++counter;
                            ...
                            cout << "Made " << counter << " Holes at row :" << row << " Total made : " << count << endl;
                            // sf::sleep(sf::milliseconds(animationDelay_ms));
                        }
                    }
                    if (count >= limit)
                ...
                for (int col = 1; col < n - 1; ++col)
                {
                    ...
                    if (isDisconnected(maze, row, col, true))
                    {
                        if ((rand() % 100) < chanceProbability(row, col, n))
                        {
                            ...
                            sf::sleep(sf::milliseconds(animationDelay_ms / (n / 10.f)));
                            // sf::sleep(sf::milliseconds(animationDelay_ms));
                            cout << "Made " << counter << " Holes at col :" << row << " Total made : " << count << endl;
                        }
                    }
                    if (count >= limit)
                        break;
...
void drawRat(sf::RenderWindow &window, int **maze, int currentRow, int currentCol, bool **visited, int n, bool IsDrawingAns)
{
    ...
    sf::RectangleShape borderIndicator(sf::Vector2f(4.f, cellSize));
    borderIndicator.setFillColor(sf::Color(100, 0, 100));
    startX2 += ColorIndicatorText[3].getLocalBounds().width + 10.f;
    borderIndicator.setPosition(startX2, window.getSize().y - 40.f);
    startX2 += borderIndicator.getLocalBounds().width + 10.f;
    info[4].setPosition(startX2, window.getSize().y - 40.f);

    window.draw(ColorIndicatorText[3]);
    window.draw(borderIndicator);
    window.draw(info[4]);
...
bool solve(sf::RenderWindow &window, int **maze, vector<pair<int, vector<char>>> &ans, vector<char> &path, bool **isVisited, int currentCol, int currentRow, int n, int &bestCost, int &shortestLength, int &costEffective, int depth = 0)
{
    if (gotSOl && isPerfect)
        return true;
    if (gotSOl && (totalCost > bestCost) && (currentPathLength > shortestLength))
        return true; // not false becz then it will be marked as dead end. but through other road the cell could lead to solution
    if (!window.isOpen())
        return false;
    ...
    if (currentCol == End[0] && currentRow == End[1]) // reached goal
    {
        ...
        if (!isPerfect)
        {
            ...
            if (totalCost < bestCost)
            {
                ...
                cout << "minimum cost is " << minimumCost;
            }
            // for shortest path
